...
class API:
    def __init__(self, device_id=DEVICE_ID):
        ...
        self.session = RateLimitedSession(rate=rate,
                                          max_tokens=10,
    ...
    async def execute(self,
        ...
        if response.get('status') == 'ACCEPTED':
            execution = Execution(response['executionId'], capability, retries,
                                  wait_time)
    ...
    async def _wait_execution(self, execution):
        ...
        while count < execution.retries:
            logger.debug('get %s %s/%s', execution.capability, count + 1,
                         execution.retries)
